...
void MCCROMA(gsl_matrix_uchar *RF, gsl_matrix_uchar *RecF, gsl_vector_int *vector) {
    ...
    for (h = 0; h < RF->size1; h += CROMABLOCKSIZE) {
        for (w = 0; w < RF->size2; w += CROMABLOCKSIZE) {

            k1 = gsl_vector_int_get(vector, vpos);
            k2 = gsl_vector_int_get(vector, vpos + 1);
            vpos += 2;

            tempV.matrix = gsl_matrix_uchar_submatrix(RF, h - k1, w + k2 , CROMABLOCKSIZE, CROMABLOCKSIZE).matrix;
