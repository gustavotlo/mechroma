...
int SDS_dic(MEPARM *meparm) {
    ...
    do {
		...
		if (flag_first_passing)
		{
				...
				total_iter--;					//decremento do total
    ...
    }while ((meparm->vh != center[0] || meparm->vw != center[1]) && total_iter > 0) ;		//continua se uma cordenada(x,y) for diferente do centro ou tem iteraçoes
